 ...
 * * Implements the standard DSU with **Path Compression** and **Union by Rank** optimizations.
 * This structure is designed for high-performance connectivity checks in the Steiner Forest Problem.
 * * @note The time complexity for operations is nearly constant, amortized O(alpha(N)), 
...
struct DSU {
     ...
     */
    bool isConnected(int source, int target) { return find(source) == find(target); }
};
