...
FileStats processFile(const std::string& filepath, const SolveMethodFunc alg, const std::vector<ConstraintFunc>& cstrs, const ValidatorFunc verify , const float alpha) {
    SteinerForest sf;
    ...
    double timeMs = 0;
    try { timeMs = sf.solve(alg, cstrs, verify, alpha); }
    catch (const std::exception& e) {
        std::cerr << "Failed to solve " << filepath << ": " << e.what() << std::endl;
        return {getFileName(filepath), sf.getOriginal().getNNode(), sf.getTerminals().size(), 0, sf.getOriginal().getTotalWeight(), 0, alpha};
    }
...
FileStats findBestAlpha(const std::string& filepath, const SolveMethodFunc alg, const std::vector<ConstraintFunc>& cstrs, const ValidatorFunc verify) {
    FileStats bestStats;
...
void printMarkdownHeader() {
    std::cout << "| File | Nodes | Terms | Ratio | Time (ms) | Best Alpha |" << std::endl;
    std::cout << "| :--- | :---: | :---: | :---: | :---: | :---: |" << std::endl;
}
...
void printFileRow(const FileStats& s) {
    ...
    std::cout << "| " << std::left << std::setw(20) << s.filename << " | "
              << std::right << std::setw(5) << s.nNodes << " | "
              << std::setw(5) << s.nTerminals << " | "
              << std::fixed << std::setprecision(4) << std::setw(7) << ratio << " | "
              << std::setprecision(2) << std::setw(9) << s.timeMs << " |"
              << std::setprecision(1) << std::setw(10) << s.alphaUsed << " |" << std::endl;
}
...
void printSummary(const std::string& sourceName, const std::vector<FileStats>& stats) {
    ...
    std::cout << "| Source | Count | Nodes | Max Ratio | Min Ratio | Most Freq Alpha |" << std::endl;
    std::cout << "| :--- | :---: | :---: | :---: | :---: | :---: |" << std::endl;
    std::cout << "| " << getFileName(sourceName) << " | "
                << count << " | "
                << minNodes << "-" << maxNodes << " | "
                << std::fixed << std::setprecision(4) << maxRatio << " | "
                << std::fixed << std::setprecision(4) << minRatio << " | "
                << std::setprecision(1) << bestAlphaGlobal << " (" << maxWins << " wins) |" << std::endl;
}