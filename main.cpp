...
// ==========================================
    ...
    void getFilesInDirectory(const std::string& dirPath, std::vector<std::string>& outFiles) {
        ...
        if (hFind != INVALID_HANDLE_VALUE) {
            do {
                ...
                if (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) getFilesInDirectory(fullFileName, outFiles);
                else outFiles.push_back(fullFileName);
        ...
        if (dir != NULL) {
            while ((ent = readdir(dir)) != NULL) {
                ...
                if (stat(fullPath.c_str(), &st) == 0) {
                    if (S_ISDIR(st.st_mode)) getFilesInDirectory(fullPath, outFiles);
                    else outFiles.push_back(fullPath);
                }
            }
            closedir(dir);
        } else {
...
int main(int argc, char** argv) {
    ...
    app.add_flag("--test-graph", flag_test_graph, "Runs only the Graph class tests");
    app.add_flag("--test-dijkstra", flag_test_dijkstra, "Runs only the Dijkstra algorithm tests");
    app.add_flag("--test-SFP", flag_test_SFP, "Runs only the Steiner Forest Problem implementation tests");
    app.add_flag("--test-GRASPCONS", flag_test_GRASPcons, "Runs only GRASP constructive heuristic tests");
    ...
    app.add_option("-f,--file", input_file, "Path to a single .stp file to solve")->check(CLI::ExistingFile);
    app.add_option("-d,--directory", input_dir, "Path to a directory containing .stp files (Without the \"\\\" or \"/\" at the end)")->check(CLI::ExistingDirectory);
    app.add_option("-a,--alpha", alpha, "Alpha parameter for constructive heuristic")->check(CLI::Range(0.0, 1.0));
    app.add_flag("-v,--variation", alpha_variation, "Test alphas [0.0, 0.1 ... 1.0] and pick best");

    CLI11_PARSE(app, argc, argv);

    if (flag_test_all || flag_test_graph || flag_test_dijkstra || flag_test_SFP || flag_test_GRASPcons) {
        if (flag_test_all) {
            graphTests();
            dijkstraTests();
            // steinerForestTests();
