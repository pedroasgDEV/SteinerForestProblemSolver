...
static std::vector<std::pair<int, int>> generatePairs(const std::vector<std::set<int>>& terminals) {
...
static std::vector<std::set<int>> preprocessTerminals(int nNodes, const std::vector<std::pair<int, int>>& terminals) {
 ...
 */
Graph GRASPconstructiveHeuristic(const Graph& originalGraph, const std::vector<std::pair<int, int>>& terminals, float alpha) {
    ...
    for(auto p : pairsRaw)
        CL.push_back({p.first, p.second, dijkstra(workingGraph, p.first, p.second).second});
    ...
    while (!CL.empty()) {
        ...
        // Sort CL
        std::sort(CL.begin(), CL.end(), [](const TerminalPair& a, const TerminalPair& b) {
        ...
        std::vector<int> path = dijkstra(workingGraph, selectedP.u, selectedP.v).first;
