...
class ConcreteSteiner : public SteinerForest {
 public:
  ...
  void setManualCost(float c) { this->totalCost = c; }

  void injectNegativeEdge() { this->originalGraph.addEdge(0, 1, -5.0f); }
 ...
 */
Graph mockHeuristic(const Graph& original,
 ...
 */
bool mockVerifierPass(const Graph& sol,
 ...
 */
bool mockVerifierFail(const Graph& sol,
                      const std::vector<std::pair<int, int>>& t) {
  return false;
}
...
void testSolveTemplate() {
  ...
  try {
    ...
    assert(sf.getSolution().isAdjacent(0, 1) ==
           true);  // Solution should be a copy of original
  } catch (...) {
  ...
  try {
    sf2.solve(mockHeuristic, {mockConstraintPass},
