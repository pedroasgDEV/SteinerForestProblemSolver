...
public:
    ...
    friend std::iostream operator << (std::ostream& out, const SFPSolution& solution);
};
 ...
 * Contains the original graph and the terminal pairs. This object should be read-only.
 */
class SFPProblem {
private:
    ...
    std::vector<int> reverseEdgeMapping; // Helper for bidirectional CSR
...
public:
    ...
    explicit SFPProblem(std::shared_ptr<Graph> g, const std::vector<std::pair<int, int>>& terminals) 
    ...
    int getReverseEdgeIdx(int edgeIdx) const;
    ...
    int getNNodes() const { return graph->getNNode(); }
